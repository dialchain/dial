...
## Registration
Network service providers must register their services with the Dial by submitting corresponding performance declarations to the Dial. Following the same rationales as explained for publishers, a network service provider must register the performance declaration for __tw__ in __tw-2__. A performance declaration is binding. In the same logic of publishers, coin earned in a time window can only be rediemed in the subsequent time window.

# Performance Considerations
## Scalability
The Dial is designed to allow for efficient participation of memory and space limited devices.
- The maintenance of two time windows of log is sufficient to keep the dial publication process ongoing
- There is no requirement for a single entity to hold all validation protocols. All partial protocols are passed over from neighborhood to neighborhood accross time windows.
- The presence of two partial merkel trees is sufficient to validate every single declaration.
- Partial protocols and publication records of relevant token must be held by current controller, as those are used to transfer control on underlying token.
...
- A network node maintaining a full dial log can validate payment locally and provide services to caller before forwarding payment token to the log.
- A more valuable service caan require the caller to get the payment published before adding the payment token to the service call.

# Security Considerations
The permissionless character of the Diaal make it difficul to manage paayable interaction processes. The Dial designs pattern used to prevent opportunistic behavior of network participants.

## Arbitrage Rules
Arbitrage rules are Dial built in validations constrain, that allow Dial network to establish a commercial environment without intermediaries.

### Redieming a Coin
In order to controll a coin earned for performing, a service provider must submit the earned coin for publication. The coin declaration must include a proof of service performance.

### Proof of Servicce Performance